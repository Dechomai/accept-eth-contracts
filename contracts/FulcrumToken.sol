...
contract FulcrumToken is ERC20, Owned {
    ...
    function approveAndInitiateExchange(
        string _initiatorItemName,
        uint256 _initiatorItemQuantity,
        string _partnerItemName,
        uint256 _partnerItemQuantity,
        uint256 _price,
        address _partner
    ) public returns (bool) {
        Exchange exchangeContract = new Exchange(
            _initiatorItemName,
            _initiatorItemQuantity,
            _partnerItemName,
            _partnerItemQuantity,
            _price,
            msg.sender,
            _partner,
            this
    ...
    function approveAndAccept(Exchange _exchangeContract)
        public
        returns (bool)
    {
    ...
    function setPrices(uint256 _fulcsPerWei) public onlyOwner {
        fulcsPerWei = _fulcsPerWei;
    }

    function buy() public payable {
        uint256 amount = msg.value * fulcsPerWei;
        if (msg.value == 0) {
            return;
        }
