...
contract Exchange {
    ...
    constructor(
        string initiatorItemName,
        uint256 initiatorItemQuantity,
        string partnerItemName,
        uint256 partnerItemQuantity,
        uint256 _price,
        address _initiator,
        address _partner,
        ERC20 _token
    ...
    function givePartnerItemFeedback(bool _isSatisfied)
        public
        onlyInitiator
        inState(State.AcceptedByPartner)
    {
    ...
    function giveInitiatorItemFeedback(bool _isSatisfied)
        public
        onlyPartner
        inState(State.AcceptedByPartner)
    {
    ...
    function checkForCompletion() private inState(State.AcceptedByPartner) {
        if (initiatorItem.hasFeedback && partnerItem.hasFeedback) {
            if (
                initiatorItem.isClientSatisfied && partnerItem.isClientSatisfied
            ) {
                require(token.approve(initiator, price));
